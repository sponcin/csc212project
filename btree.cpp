...
void BTree::insert(int theKey, std::string theWord) {
    ...
    if (root == nullptr) {
        ...
        if (root->currentKeys == 2*minDeg - 1) {
            ...
            //since we know the neither of newRoot's children are full, we insert the key, word pair here.
...
void BTree::callUserSearch(int key, bool &isPresent, int &theCount) {
    userSearch(key,root,isPresent,theCount);
    return;
}



void BTree::userSearch(int key, BTNode* root, bool &isPresent, int &theCount) {
    //start at first index of the data vector in current node
    int index = 0;
    ...
    if (index < root->currentKeys && key == root->data[index].key) {
        //this word is in the tree, boolean is now true so insert will not occur
        //assign count of found word to theCount
        isPresent = true;
        theCount = root->data[index].count;
        return;
    }
    ...
    //if neither base case is hit, we recurse down a level, travelling to the proprer child node
    return userSearch(key, root->children[index], isPresent, theCount);
}
