...
//traverses through tree and to search for user input word
void BTree::traverseSearch(BTNode* node, int key, int &counter) {

    //loop of recursive calls provided the node is not a leaf
    for (int i = 0; i <= node->currentKeys; i++) {
        if(!node->isLeaf) {
            traverseSearch(node->children[i], key, counter);
        }
        if(key == node->data[i].key){
            std::cout << node->data[i].word << " was in the file " << node->data[i].count << " times." << std::endl;
            counter += 1;
        }
    }
}

//public facing function that traverses the tree and says if the word is found or not
void BTree::callTraverseSearch(int key) {
    int counter = 0;
    traverseSearch(root, key, counter);
    if(counter == 0)
    {
        std::cout << "I'm sorry, that word is not in the file." << std::endl;
    }
    return;
}
